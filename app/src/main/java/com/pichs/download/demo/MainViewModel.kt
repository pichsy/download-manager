...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * 模拟批量后台下载
     * 使用新的 preCheckDownload API 实现"先决策后创建任务"
     */
    fun simulateBatchDownload() {
        viewModelScope.launch {
            ...
            // 使用新的预检查 API
            when (val result = DownloadManager.preCheckDownload(totalSize)) {
                ...
                is com.pichs.download.model.PreCheckResult.WifiOnly -> {
                    _uiEvent.emit(UiEvent.ShowToast("当前设置为仅 WiFi 下载，请连接 WiFi"))
                }
