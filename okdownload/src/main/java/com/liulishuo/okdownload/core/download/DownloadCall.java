    ...
    public void execute() throws InterruptedException {
        ...
        do {
            ...
            try {
                ...
                Util.d(TAG, "Download failed due to IOException: " + e.getMessage());
                cache.setUnknownError(e);
                // 增加重试逻辑
                if (reCount < 3) {
                    reCount++;
                    Util.d(TAG, "Retrying download, attempt " + reCount);
                    continue;
                } else {
                    Util.d(TAG, "Max retry attempts reached, giving up");
                    break;
                }
            } catch (Exception e) {
                Util.d(TAG, "Download failed due to unexpected error: " + e.getMessage());
                cache.setUnknownError(new IOException("unknown exception: ",e));
                break;
            }
